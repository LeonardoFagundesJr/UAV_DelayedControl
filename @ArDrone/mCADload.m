function mCADload(drone)
% Load ArDrone 3D CAD model

drone.pCAD.obj = loadawobj('ArDroneCAD.obj');
drone.pCAD.mtl = loadawmtl('ArDroneCAD.mtl');

% Flags
drone.pCAD.flagLoaded  = 1;  % 3D image loaded
drone.pCAD.flagCreated = 0;  % 3D image created using Matlab Patch function
drone.pCAD.flagLines   = 1;  % Illustrate lines between ArDrone and surface

vExt = minmax(drone.pCAD.obj.v);
mExt = mean(vExt,2);
for ii = 1:3
    drone.pCAD.obj.v(ii,:) = (drone.pCAD.obj.v(ii,:) - mExt(ii))/1000;
end

drone.pCAD.obj.v = [0 1 0; -1 0 0; 0 0 1]*drone.pCAD.obj.v;
