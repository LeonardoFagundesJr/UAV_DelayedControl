...
mkdir(pwd,'TrajectoryTrack_Lemn');
...
for ii = 1:length(MAPA)
    ...
    while t < tmax
        ...
        A.pPos.Xd(1) = rx*sin(w*t);      % posição x  
        A.pPos.Xd(2) = ry*sin(2*w*t);    % posição y    
        A.pPos.Xd(3) = 1+rz*sin(w*t);    % posição z

        A.pPos.Xd(7) = rx*w*cos(w*t);       % dx
        A.pPos.Xd(8) = 2*ry*w*cos(2*w*t);   % dy
        A.pPos.Xd(9) = rz*w*cos(w*t);
...
cd(strcat(pwd,'\TrajectoryTrack_Lemn'))
